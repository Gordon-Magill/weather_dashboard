...
function roundSigFigs(numberArg,sigFigs) {
    return Math.round((numberArg+Number.EPSILON)*(10**sigFigs))/(10**sigFigs);
}
...
function renderForecastConditions(data) {
    ...
    for (i=0;i<5;i++) {
        ...
        curTemperatureEl.text(`Temp(\u2103): ${roundSigFigs(data.daily[i].temp.day,1)}(Day) / ${roundSigFigs(data.daily[i].temp.night,1)}(Night)  `);
    
        var curHumidityEl = $(`#humidityEl${i}`);
        curHumidityEl.text(`Humidity: ${data.daily[i].humidity}%`)
    
        var curWindEl = $(`#windEl${i}`);
        curWindEl.text(`Wind speed: ${roundSigFigs(data.daily[i].wind_speed,1)}m/s`)
